...
- The gate (every iteration): `dart pub get && dart analyze && dart test`.
- Coverage ≥ 80% on `didwebvh_core` (Codecov); `signing_local` and `wizard` excluded, as in Java.

## CI/CD

- **ci.yml:** on push to `main` and all PRs; SDK matrix (stable / declared minimum / beta);
  `dart analyze` + `dart test --coverage` → Codecov.
...
1. **Update `CHANGELOG.md`** under `## [Unreleased]` (Added / Changed / Fixed), referencing the spec section
   or the ported Java class for non-obvious behaviour.
2. **Propose** a [Conventional Commits](https://www.conventionalcommits.org/) message — and **stop**.
...
5. Run the gate; report the real result; copy the shared vectors where the iteration calls for it.
