...
| **HTTP (resolution)** | OkHttp (10s timeout, 200KB cap) | [`http`](https://pub.dev/packages/http) (official) behind a `RemoteDidFetcher` interface | Official, minimal. Enforce timeout + size cap in the fetcher, same as Java. **Streamed cap (iteration 9):** `HttpDidFetcher` uses `Client.send()` + `StreamedResponse.stream`, summing chunk lengths and aborting once the byte count exceeds `maxResponseSize` — the body is rejected **while reading**, never fully buffered (a `Content-Length` over the cap short-circuits first). This reproduces OkHttp's streamed rejection and keeps the cap meaningful even when the host omits or lies about `Content-Length`; a HEAD precheck was rejected as unreliable/spoofable. |
...
| **Lint / static analysis** | Checkstyle + SpotBugs | `dart analyze` + [`very_good_analysis`](https://pub.dev/packages/very_good_analysis) | VGA is the strictest curated rule set (~strict-mode + pedantic); single config replaces both Checkstyle and SpotBugs. **Version pin (iteration 9):** pinned to `^7.0.0` — the newest VGA line that supports the `^3.6.0` SDK floor (see §3). VGA `8.0.0+` requires Dart `>=3.7.0`, which would force the floor up and break the `3.6.0` CI leg; keeping `^7.0.0` preserves the Affinidi-`ssi`-aligned `^3.6.0` floor. Revisit if/when the floor is intentionally raised. |
