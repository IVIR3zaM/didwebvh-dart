...
> **Note for the Dart port.** This document describes the architecture of the **reference Java
> implementation** (`didwebvh-java`). It is preserved here verbatim because the high-level design, the
> spec algorithms (SCID, entry hash, proofs, log-chain validation, pre-rotation), and the package
