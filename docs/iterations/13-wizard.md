...
- **Two new public core exports.** The wizard needs `Base58Btc.encodeMultibase` (witness signature encoding) and
  ...
  `didwebvh-core` that the Java wizard imports. Added them to the `didwebvh` barrel so the wizard uses the
...
- **No new core logic**, so `didwebvh` coverage is unchanged (the two added barrel lines re-export
